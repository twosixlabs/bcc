...
class KillSnoop:
    ...
    def __init__(self, args, debug=0):
        ...
        def print_event(cpu, data, size):
            ...
            if args.json:
                printb(b'{"time": %s, "pid": %d, "command": %s, "thread": %d, "signal": %d, "ret": %d}' % (strftime("%H:%M:%S"),
                                              event.pid, event.comm,
                                              event.sig, event.tpid,
                                                                                                           event.ret))
            else:
...
def client_main(args):
    ...
    examples = """examples:
        ...
        ./killsnoop -j        # print output as JSON objects
    """
