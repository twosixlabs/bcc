...
# 14-08-2016    Michael D. Day II converted to a class, added JSON output
...
class ExecSnoop:
    ...
    def __init__(self, args):
        ...
        self.bpf_text = self.bpf_text.replace("MAXARG", args.max_args)
        if args.ebpf:
            print(bpf_text)
            exit()
        ...
        # header, do not print for JSON output
        if not args.json:
            if args.timestamp:
                print("%-8s" % ("TIME(s)"), end="")
